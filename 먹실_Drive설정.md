...
코드는 그대로고, 아래 값을 준비하면 PC·폰 어디서든 같은 드라이브 폴더로 집필할 수 있어.
...
1. **프로젝트 생성** (이름 아무거나, 예: `muksil`).
   - 나중에 먹실 설정에 넣을 **프로젝트 번호**도 메모해둬. Google Cloud 상단 프로젝트 선택 메뉴나 "IAM 및 관리자 → 설정"에서 확인할 수 있어.
...
4. **사용자 인증 정보 → 사용자 인증 정보 만들기**:
   ...
   - **API 키**도 하나 더 만들기 (Picker 폴더 선택에 필요).
     - 처음 테스트할 때는 제한 없이 연결 확인.
     - 성공 후 제한을 걸려면 애플리케이션 제한은 **HTTP 리퍼러**(`https://도메인/*`), API 제한은 **Google Drive API**로 두면 됨.
...
2. **OAuth Client ID**, **API Key**, **프로젝트 번호(Picker App ID)** 붙여넣고 저장.
3. **Drive 연결** → 구글 로그인/동의 (테스트 사용자라 "확인되지 않은 앱" 경고가 떠도 *고급 → 이동*으로 통과).
4. **Muksil 폴더 자동 연결** → Drive 최상위에 `Muksil` 폴더를 만들거나 기존 폴더를 찾아 연결.
   - 원하는 폴더를 직접 고르고 싶으면 **폴더 선택**을 써도 됨.
   - 폴더 선택에서 API Key 오류가 나면 자동 연결을 쓰면 됨.
...
- **Client ID / API Key / 프로젝트 번호**: 이 브라우저(localStorage)에만 저장돼. 기기 바꾸면 설정에서 다시 입력.
