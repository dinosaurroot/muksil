...
- DB: sql.js(SQLite WASM), 메모리 인스턴스 `db`. 테이블: works/chapters/characters/lore/plots/timeline/notes/writing_log/cats/attachments/meta (cats=공통 카테고리 kind=lore|plot|char|event; timeline=연표; attachments=첨부 메타(파일은 `<작품제목>/<메뉴폴더>/`에 저장, DB는 참조만); lore_cats는 레거시).
...
- claude.ai 아티팩트 프리뷰에선 localStorage/FS/Drive 전부 미동작 → **독립 실행/호스팅에서 테스트**.
